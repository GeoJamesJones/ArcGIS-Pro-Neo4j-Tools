...
def main():
    starting_node = arcpy.GetParameterAsText(0)
    upstream = arcpy.GetParameterAsText(1)
    downstream = arcpy.GetParameterAsText(2)
    output_workspace = arcpy.GetParameterAsText(3)
    desc = arcpy.Describe(output_workspace)

    if int(upstream) == 0 and int(downstream) > 0:
        base_cypher = "MATCH (source:Airport{code:'" + starting_node + "'}) -[*1.."+ downstream + "]-> (target) RETURN DISTINCT source,target;"
    elif int(upstream) > 0 and int(downstream) == 0:
        base_cypher = "MATCH (target) -[*1.."+ upstream + "]-> (source:Airport{code:'" + starting_node + "'}) RETURN DISTINCT source,target;"
    else:
        base_cypher = "MATCH (target) -[*1.."+ upstream + "]-> (source:Airport{code:'" + starting_node + "'}) -[*1.."+ downstream + "]-> (target) RETURN DISTINCT source,target;"

    arcpy.AddMessage(base_cypher)
   ...
   # Sets the spatial reference for the output feature class to WGS 1984.
    ...
    arcpy.CreateFeatureclass_management(output_workspace, 'Nodes', "POINT", None, "DISABLED", "DISABLED", sr)
    arcpy.CreateFeatureclass_management(output_workspace, 'Edges', "POLYLINE", None, "DISABLED", "DISABLED", sr)
    node_fc = os.path.join(output_workspace, "Nodes")
    edge_fc = os.path.join(output_workspace, "Edges")
    # Adds the necessary fields to the feature class
    arcpy.management.AddFields(node_fc, "name TEXT # 255 # #;location TEXT # 255 # #;state TEXT # 255 # #;country TEXT # 255 # #;start_date DATE;code TEXT # 10 # #")
    arcpy.management.AddFields(edge_fc, "source_node TEXT 'Source Node' 255 # #;target_node TEXT 'Target Node' 255 # #")
    
    # Fields to be edited when creating the Insert Cursor objects for the node and edge tables
    node_fields = ["SHAPE@XY", "name", "location", "state", "country", "start_date", "code"]
    edge_fields = ["SHAPE@", "source_node", "target_node"]
    ...
    for r in results:
        ...
        row = [line,source['name'],target['name']]
