...
def main():
    ...
    start_node = arcpy.GetParameterAsText(0)
    end_node = arcpy.GetParameterAsText(1)
    output_workspace = arcpy.GetParameterAsText(2)
    desc = arcpy.Describe(output_workspace)

    base_cypher = "MATCH (s:Airport { code: '" + start_node + "'}),(t:Airport { code: '" + end_node + "'}), p = shortestPath((s)-[:CONNECTION*]-(t)) WHERE ALL (r IN relationships(p) WHERE exists(r.airline)) RETURN p"
